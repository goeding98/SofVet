...
- **Cita con Especialistas** (`/citas-especialistas`) — CitasEspecialistasPage.jsx: tablero Kanban (4 columnas: Solicitada → Confirmada por tutor y especialista → Pagada y programada, más una columna aparte de Desestimiento) para coordinar remisiones a especialistas externos sin depender del grupo de WhatsApp. Cada sede monta la solicitud (especialidad, tutor, mascota, motivo, urgencia); al arrastrar a "Confirmada" pide obligatoriamente el nombre del especialista confirmado; al arrastrar a "Programada" recuerda agendar la cita real en `/appointments`; si el tutor cancela después de confirmada (o en cualquier punto), la card se arrastra a "Desestimiento". Guarda fecha/hora/usuario de cada etapa (solicitada/confirmada/programada/desestimiento) en la tabla `citas_especialistas` para poder medir tiempos y volumen por especialidad más adelante. Las columnas "Programada" y "Desestimiento" solo pintan las 15 más recientes cada una (nada se borra). Admin y Laboratorio ven todas las sedes (con filtro propio en la página, además del selector de sede del Navbar); el resto solo ve la suya.
