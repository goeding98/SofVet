...
- **Cita con Especialistas** (`/citas-especialistas`) — CitasEspecialistasPage.jsx: tablero Kanban (3 columnas: Solicitada → Confirmada por tutor y especialista → Pagada y programada) para coordinar remisiones a especialistas externos sin depender del grupo de WhatsApp. Cada sede monta la solicitud (especialidad, tutor, mascota, motivo, urgencia); al arrastrar a "Confirmada" pide obligatoriamente el nombre del especialista confirmado; al arrastrar a "Programada" recuerda agendar la cita real en `/appointments`. Guarda fecha/hora/usuario de cada etapa (solicitada/confirmada/programada) en la tabla `citas_especialistas` para poder medir tiempos y volumen por especialidad más adelante. Columna "Programada" solo pinta las 15 más recientes (nada se borra). Admin y Laboratorio ven todas las sedes (con el selector de sede del Navbar); el resto solo ve la suya.
