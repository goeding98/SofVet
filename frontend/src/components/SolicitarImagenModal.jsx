...
export default function SolicitarImagenModal({ isOpen, onClose, onSave, pet }) {
  ...
  const handleSave = () => {
    if (tipo === 'Otro' && !otroTipo.trim()) { alert('Especifica el tipo de imagen.'); return; }
    onSave({
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem 1rem', backdropFilter:'blur(2px)' }}>
      <div onClick={e => e.stopPropagation()} style={{ background:'var(--color-white)', borderRadius:'var(--radius-xl)', boxShadow:'var(--shadow-lg)', width:'100%', maxWidth:420, overflow:'hidden' }}>
        ...
        <div style={{ padding:'1.25rem 1.5rem', borderBottom:'1px solid var(--color-border)', background:'#e8f0ff', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            <h3 style={{ fontFamily:'var(--font-title)', color:'#1565c0', fontSize:'1.1rem', margin:0 }}>🔬 Solicitar Imagen</h3>
        ...
        <div style={{ padding:'1.5rem', display:'flex', flexDirection:'column', gap:'1rem' }}>
          <div>
            <label style={lSt}>Tipo de imagen *</label>
            <select value={tipo} onChange={e => setTipo(e.target.value)} style={iSt}>
              {TIPOS_IMAGEN.map(t => <option key={t}>{t}</option>)}
            </select>
            {tipo === 'Otro' && (
              <input value={otroTipo} onChange={e => setOtroTipo(e.target.value)} placeholder="¿Cuál imagen?" style={{ ...iSt, marginTop:'0.5rem', border:'1px solid #1565c0' }} autoFocus />
            )}
          </div>
          ...
          <div style={{ display:'flex', gap:'0.75rem', justifyContent:'flex-end' }}>
            ...
            <button onClick={handleSave} style={{ padding:'0.6rem 1.5rem', background:'#1565c0', color:'white', border:'none', borderRadius:'var(--radius-md)', cursor:'pointer', fontFamily:'var(--font-body)', fontSize:'0.875rem', fontWeight:600 }}>
              🔬 Solicitar imagen
            </button>
          </div>
        </div>
