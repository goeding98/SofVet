...
export default function ImagenesResultModal({ isOpen, onClose, onSave, onEdit, pet, pedidos = [], initialData = null, imgId = null }) {
      ...
      if (isEditMode) {
        ...
        setExistingArchivos(
          initialData.archivos?.length > 0
            ...
            : initialData.file_url ? [{ name: 'Archivo adjunto', url: initialData.file_url }] : []
        );
        setSelectedPedidoId('');
      } else {
        setResult('');
  ...
  const handleFiles = (e) => {
    ...
    const oversize = selected.filter(f => f.size > 20 * 1024 * 1024);
    if (oversize.length) { setError(`Archivo(s) demasiado grande(s): ${oversize.map(f => f.name).join(', ')} (máx 20MB c/u)`); return; }
    setFiles(prev => {
  ...
  const handleNewSave = async () => {
    ...
    if (solicitados.length > 1 && !selectedPedidoId) { setError('Selecciona a qué pedido corresponde esta imagen.'); return; }
    setError('');
    let archivos = [];
    if (files.length > 0) {
      setUploading(true);
      ...
      <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem 1rem', backdropFilter:'blur(2px)' }}>
        <div onClick={e => e.stopPropagation()} style={{ background:'var(--color-white)', borderRadius:'var(--radius-xl)', boxShadow:'var(--shadow-lg)', width:'100%', maxWidth:440, overflow:'hidden' }}>
          <div style={{ padding:'1.25rem 1.5rem', borderBottom:'1px solid var(--color-border)', background:'#fff8e1', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <h3 style={{ fontFamily:'var(--font-title)', color:'#b8860b', fontSize:'1.1rem', margin:0 }}>🔬 Subir Imagen</h3>
          ...
          <div style={{ padding:'2rem 1.5rem', textAlign:'center' }}>
            ...
            <p style={{ fontWeight:600, color:'var(--color-text)', marginBottom:'0.5rem' }}>No hay imágenes solicitadas para <strong>{pet.name}</strong>.</p>
            <p style={{ fontSize:'0.82rem', color:'var(--color-text-muted)', marginBottom:'1.5rem' }}>Primero solicita una imagen usando <strong>"Solicitar imagen"</strong>.</p>
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'flex-start', justifyContent:'center', padding:'2rem 1rem', backdropFilter:'blur(2px)', overflowY:'auto' }}>
      <div onClick={e => e.stopPropagation()} style={{ background:'var(--color-white)', borderRadius:'var(--radius-xl)', boxShadow:'var(--shadow-lg)', width:'100%', maxWidth:540, margin:'auto', overflow:'hidden' }}>
        ...
        <div style={{ padding:'1.25rem 1.5rem', borderBottom:'1px solid var(--color-border)', background: isEditMode ? '#fff8f0' : '#e8f0ff', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            <h3 style={{ fontFamily:'var(--font-title)', color: isEditMode ? '#b45309' : '#1565c0', fontSize:'1.1rem', margin:0 }}>
              {isEditMode ? '✏️ Editar Imagen' : '🔬 Subir Imagen'}
            </h3>
        ...
        <div style={{ padding:'1.5rem' }}>
          ...
          {!isEditMode && (
            <div style={{ marginBottom:'1rem' }}>
              <label style={lSt}>¿A qué pedido corresponde? *</label>
              {solicitados.length === 1 ? (
                <div style={{ padding:'0.75rem 1rem', background:'#e8f0ff', border:'1px solid #1565c0', borderRadius:'var(--radius-sm)', display:'flex', alignItems:'center', gap:'0.6rem' }}>
                  <span style={{ fontSize:'1.1rem' }}>🔬</span>
                  <div>
                    <div style={{ fontWeight:700, fontSize:'0.875rem', color:'#1565c0' }}>{solicitados[0].tipo_examen}</div>
                ...
                <select value={selectedPedidoId} onChange={e => setSelectedPedidoId(e.target.value)} style={{ ...iSt, border:'1px solid #1565c0' }}>
          ...
          <div style={{ marginBottom:'1rem' }}>
            <label style={lSt}>Hallazgos / Interpretación</label>
            <textarea value={resultados} onChange={e => setResult(e.target.value)} rows={3} style={taSt} placeholder="Describa los hallazgos o interpretación de la imagen..." />
          </div>

          {isEditMode && existingArchivos.length > 0 && (
            <div style={{ marginBottom:'1rem' }}>
              <label style={lSt}>Archivos actuales</label>
              <div style={{ display:'flex', flexDirection:'column', gap:'0.4rem' }}>
                {existingArchivos.map((f, i) => (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:'0.5rem', padding:'0.5rem 0.75rem', background:'#e8f0ff', border:'1px solid #1565c0', borderRadius:'var(--radius-sm)' }}>
                    <a href={f.url} target="_blank" rel="noopener noreferrer" style={{ fontSize:'0.82rem', color:'#1565c0', fontWeight:600, flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', textDecoration:'none' }}>
                      📎 {f.name}
                    </a>
                    <button onClick={() => removeExistingFile(i)} style={{ background:'var(--color-danger-bg)', border:'1px solid var(--color-danger)', color:'var(--color-danger)', borderRadius:'var(--radius-sm)', cursor:'pointer', fontSize:'0.72rem', padding:'2px 8px', flexShrink:0, fontFamily:'var(--font-body)' }}>
                      🗑 Eliminar
                    </button>
                  </div>
                ))}
          ...
          <div style={{ marginBottom:'1.25rem' }}>
            <label style={lSt}>{isEditMode ? 'Agregar nuevos archivos' : 'Adjuntar archivos (PDF, JPG, PNG)'}</label>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'0.4rem', marginBottom:'0.5rem' }}>
                {files.map((f, i) => (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:'0.5rem', padding:'0.5rem 0.75rem', background:'#fff8e1', border:'1px solid #f59e0b', borderRadius:'var(--radius-sm)' }}>
                    <span style={{ fontSize:'0.82rem', color:'#b45309', fontWeight:600, flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                      📎 {f.name} <span style={{ fontWeight:400, color:'var(--color-text-muted)' }}>({(f.size / 1024).toFixed(0)} KB)</span>
                    </span>
          ...
          <div style={{ display:'flex', gap:'0.75rem', justifyContent:'flex-end' }}>
            ...
            <button onClick={isEditMode ? handleEditSave : handleNewSave} disabled={uploading}
              style={{ padding:'0.6rem 1.5rem', background: isEditMode ? '#b45309' : '#1565c0', color:'white', border:'none', borderRadius:'var(--radius-md)', cursor: uploading ? 'not-allowed' : 'pointer', fontFamily:'var(--font-body)', fontSize:'0.875rem', fontWeight:600, opacity: uploading ? 0.7 : 1 }}>
              {uploading
