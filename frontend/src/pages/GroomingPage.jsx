...
const GROOMING_SERVICES = [
  'Baño', 'Baño + Corte', 'Corte de pelo', 'Deslanado',
...
// ── Block colors (fixed purple for grooming) ──────────────────────────────────
const BLOCK_COLOR = '#7c5cbf';
const BLOCK_BG    = '#f0ebff';

// ── Helpers ──────────────────────────────────────────────────────────────────
const mkForm = (date, time) => ({
  patient_name:'', owner:'', services:[],
  date:date||'', time:time||'09:00', time_end:addMins(time||'09:00', 60),
...
function GroomBlock({ item, isAdmin, onEdit, onDelete, onStatusChange }) {
  ...
  return (
    <div
      onClick={e => { e.stopPropagation(); isAdmin && onEdit(item); }}
    ...
    >
      ...
      {st.next && (
        <button
          onClick={e => { e.stopPropagation(); onStatusChange(item, st.next); }}
...
function PhotoField({ label, value, onChange }) {
  ...
  const handleFile = async (e) => {
    ...
    if (!error) {
      ...
      // Fallback: store as local object URL for this session
...
export default function GroomingPage() {
  ...
  useSede(); // keep context subscription
  ...
  const openEdit = (item) => { setForm({...item, services: Array.isArray(item.services)?item.services:(item.services?[item.services]:[]) }); setEditId(item.id); setModal(true); };
  const handleSave = () => {
    if (!form.patient_name?.trim()||!form.date) { alert('Paciente y fecha son requeridos'); return; }
  ...
  const viewBtn = (mode, label) => (
    <button onClick={()=>setViewMode(mode)} style={{ padding:'0.35rem 0.9rem', border:'1px solid', borderColor:viewMode===mode?BLOCK_COLOR:'var(--color-border)', borderRadius:'var(--radius-sm)', background:viewMode===mode?BLOCK_COLOR:'var(--color-white)', color:viewMode===mode?'white':'var(--color-text-muted)', fontFamily:'var(--font-body)', fontSize:'0.78rem', fontWeight:600, cursor:'pointer' }}>
      {label}
    </button>
  );

  // ── Month view rendered inline ─────────────────────────────────────────────
                ...
                >
                  {dayNum&&<>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'0.2rem'}}>
                      ...
                      <span onClick={e=>{e.stopPropagation();openAdd(ds);}} style={{fontSize:'0.85rem',color:'var(--color-border)',cursor:'pointer',opacity:0.5,userSelect:'none'}}>+</span>
                    </div>
        ...
        <div style={{background:'var(--color-white)',border:'1px solid var(--color-border)',borderRadius:'var(--radius-xl)',overflow:'hidden',boxShadow:'var(--shadow-sm)',position:'sticky',top:'1rem'}}>
          ...
          <div style={{padding:'0.65rem',maxHeight:'65vh',overflowY:'auto'}}>
            {dayItems.length===0?(
              ...
              return (
                <div key={it.id} style={{border:'1px solid var(--color-border)',borderLeft:`3px solid ${st.color}`,borderRadius:'var(--radius-md)',padding:'0.65rem',background:'var(--color-white)',marginBottom:'0.5rem'}}>
                  ...
                  <div style={{fontWeight:700,fontSize:'0.82rem',marginBottom:'0.1rem'}}>✂️ {it.patient_name}</div>
                  {it.owner&&<div style={{fontSize:'0.7rem',color:'var(--color-text-muted)'}}>👤 {it.owner}</div>}
                  ...
                  {(it.photo_before||it.photo_after)&&(
                    <div style={{display:'flex',gap:'0.4rem',marginTop:'0.4rem'}}>
                      {it.photo_before&&<img src={it.photo_before} alt="antes" style={{width:48,height:48,objectFit:'cover',borderRadius:4,border:'1px solid var(--color-border)'}} title="Foto antes"/>}
                      {it.photo_after&&<img src={it.photo_after} alt="después" style={{width:48,height:48,objectFit:'cover',borderRadius:4,border:'1px solid var(--color-border)'}} title="Foto después"/>}
                    </div>
                  )}
                  <div style={{display:'flex',gap:'0.35rem',marginTop:'0.5rem',justifyContent:'space-between',alignItems:'center'}}>
                    ...
                    <div style={{display:'flex',gap:'0.25rem',marginLeft:'auto'}}>
                      <button onClick={()=>openEdit(it)} style={{padding:'0.2rem 0.55rem',border:'1px solid var(--color-border)',borderRadius:'var(--radius-sm)',background:'var(--color-white)',cursor:'pointer',fontFamily:'var(--font-body)',fontSize:'0.67rem'}}>✏️</button>
                      <button onClick={()=>handleDelete(it)} style={{padding:'0.2rem 0.55rem',border:'1px solid var(--color-danger)',borderRadius:'var(--radius-sm)',background:'var(--color-white)',cursor:'pointer',fontFamily:'var(--font-body)',fontSize:'0.67rem',color:'var(--color-danger)'}}>🗑</button>
                    </div>
                  </div>
                </div>
              );
    ...
    <div>
      ...
      <div style={{display:'flex',gap:'0.75rem',alignItems:'center',flexWrap:'wrap',marginBottom:'1rem',justifyContent:'space-between'}}>
        ...
        <div style={{display:'flex',gap:'0.4rem',flexWrap:'wrap',alignItems:'center'}}>
          ...
          {SEDES.map(s=>{const active=sedeFilter===s.id,locked=!isAdminUser&&session?.sede_id===s.id;return <button key={s.id} onClick={()=>isAdminUser?setSedeFilter(active?null:s.id):undefined} style={{padding:'0.28rem 0.75rem',borderRadius:999,fontSize:'0.72rem',fontWeight:600,cursor:isAdminUser?'pointer':'default',border:`1px solid ${active?s.color:'var(--color-border)'}`,background:active?s.color:s.bg,color:active?'white':s.color,opacity:(!isAdminUser&&!locked)?0.4:1}}>📍 {s.nombre}{locked?' · Mi sede':''}</button>;})}
        </div>
        ...
        <div style={{display:'flex',gap:'0.5rem',alignItems:'center',flexWrap:'wrap'}}>
          ...
          <button onClick={()=>openAdd(viewMode==='day'?anchor:todayStr)} style={{padding:'0.38rem 1rem',background:BLOCK_COLOR,color:'white',border:'none',borderRadius:'var(--radius-sm)',cursor:'pointer',fontFamily:'var(--font-body)',fontSize:'0.82rem',fontWeight:700}}>
            + Nuevo servicio
          </button>
        </div>
      </div>
      ...
      {viewMode==='day'&&<TimeGrid days={[anchor]} byDate={byDate} todayStr={todayStr} isAdmin={isAdminUser} onCellClick={openAdd} onEdit={openEdit} onDelete={handleDelete} onStatusChange={handleStatusChange}/>}
      ...
      <Modal isOpen={modal} onClose={()=>setModal(false)} title={editId?'Editar Servicio':'✂️ Nuevo Servicio de Peluquería'} onSave={handleSave} size="md">
        ...
        <div style={{marginBottom:'1rem'}}>
          <label style={labelSt}>Servicios (puede seleccionar varios)</label>
          <div style={{display:'flex',flexWrap:'wrap',gap:'0.4rem',marginTop:'0.35rem'}}>
            {GROOMING_SERVICES.map(svc => {
              const selected = Array.isArray(form.services) && form.services.includes(svc);
              return (
                <button key={svc} type="button" onClick={()=>toggleService(svc)}
                  style={{padding:'0.35rem 0.85rem',borderRadius:999,fontSize:'0.78rem',fontWeight:600,cursor:'pointer',border:`1px solid ${selected?BLOCK_COLOR:'var(--color-border)'}`,background:selected?BLOCK_COLOR:'var(--color-white)',color:selected?'white':BLOCK_COLOR,transition:'var(--transition)'}}
                >
                  {selected?'✓ ':''}{svc}
                </button>
              );
